...
def main(args) -> None:
    ...
    model.load_state_dict(
        torch.load(args.weight_path, map_location=device), strict=False
    )
    model.to(device)
    ...
    print(
        "Load Textual Scene Graph parser from the checkpoint {}.".format(
            args.parser_checkpoint
        )
    )

    # prepare memory bank
    memory_id = args.memory_id
    memory_caption_path = os.path.join(
        f"data/memory/{memory_id}", "memory_captions.json"
    )
    memory_clip_embedding_file = os.path.join(
        f"data/memory/{memory_id}", "memory_clip_embeddings.pt"
    )
    memory_wte_embedding_file = os.path.join(
        f"data/memory/{memory_id}", "memory_wte_embeddings.pt"
    )
    ...
    if args.using_hard_prompt:
        batch_image_embeds = vl_model.compute_image_representation_from_image_path(
            args.image_path
        )
        ...
        select_memory_ids = clip_score.topk(args.memory_caption_num, dim=-1)[1].squeeze(
            0
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--parser_checkpoint",
        type=str,
        default=r"lizhuang144/flan-t5-base-VG-factual-sg",
    )
    parser.add_argument(
        "--wte_model_path", type=str, default=r"sentence-transformers/all-MiniLM-L6-v2"
    )
    ...
    parser.add_argument(
        "--weight_path", default="checkpoints/train_coco/coco_prefix-0014.pt"
    )
    parser.add_argument(
        "--image_path", default="image_example/COCO_val2014_000000027440.jpg"
    )
    ...
    parser.add_argument(
        "--memory_caption_path",
        type=str,
        default="data/memory/coco/memory_captions.json",
    )
