...
if __name__ == "__main__":
    ...
    logger.logger.info(
        "Load BartForTextInfill from the checkpoint {}.".format(args.lm_model_path)
    )
    ...
    logger.logger.info(
        "Load sentenceBERT from the checkpoint {}.".format(args.wte_model_path)
    )
    ...
    if "bart" in tokenizer.__class__.__name__.lower():
        ...
        with open(filename, "r") as fr:
            for line in fr:
                ...
                index += 1
        print(
            "Loading {} stop tokens from {} for {}.".format(
                index, filename, tokenizer.__class__.__name__
            )
        )
        ...
        with open(filename, "r") as fr:
            for line in fr:
                ...
                index += 1
        print(
            "Loading {} sub tokens from {} for {}.".format(
                index, filename, tokenizer.__class__.__name__
            )
        )
    ...
    result_dict = {}
    for batch_idx, (batch_image_embeds, batch_name_list, batch_img_list) in enumerate(
        train_loader
    ):
        start = time.time()
        logger.logger.info(
            f"{batch_idx + 1}/{len(train_loader)}, image name: {batch_name_list[0]}"
        )
        ...
        if args.use_memory:
            ...
            select_memory_ids = clip_score.topk(args.memory_caption_num, dim=-1)[
                1
            ].squeeze(0)
